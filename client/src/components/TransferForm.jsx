...
const TransferForm = ({ onTransferSuccess }) => {
  ...
  const handlePreSubmit = (e) => {
    ...
    currentRequestIdRef.current = `${Date.now()}-${Math.random().toString(36).substring(2, 10)}`;
  ...
  const executeTransfer = useCallback(async () => {
    ...
    // Use persisted requestId for retries of the same intent
    ...
    <>
      ...
      {status === 'success' && transactionData && (
        <div className="printable-receipt" style={{ display: 'none' }}>
          <div style={{ textAlign: 'center', marginBottom: '24px', paddingBottom: '16px', borderBottom: '2px solid #8b5cf6' }}>
            <h1 style={{ fontSize: '20px', margin: '0 0 4px', color: '#8b5cf6' }}>SecureBank</h1>
            <p style={{ margin: 0, fontSize: '12px', color: '#666' }}>Transaction Receipt</p>
          </div>
          <div style={{ background: '#ecfdf5', color: '#059669', padding: '8px 16px', borderRadius: '8px', textAlign: 'center', fontWeight: 600, margin: '16px 0' }}>
            ✓ Transfer Successful
          </div>
          <div style={{ display: 'flex', justifyContent: 'space-between', padding: '10px 0', borderBottom: '1px solid #f0f0f0', fontSize: '13px' }}>
            <span style={{ color: '#666' }}>Reference</span>
            <span style={{ fontWeight: 600 }}>{transactionData.reference}</span>
          </div>
          <div style={{ display: 'flex', justifyContent: 'space-between', padding: '10px 0', borderBottom: '1px solid #f0f0f0', fontSize: '13px' }}>
            <span style={{ color: '#666' }}>Transaction ID</span>
            <span style={{ fontWeight: 600 }}>{transactionData.transactionId}</span>
          </div>
          <div style={{ display: 'flex', justifyContent: 'space-between', padding: '10px 0', borderBottom: '1px solid #f0f0f0', fontSize: '13px' }}>
            <span style={{ color: '#666' }}>Amount</span>
            <span style={{ fontWeight: 600, fontSize: '18px', color: '#1a1a2e' }}>
              ${Number(transactionData.amount).toLocaleString('en-US', { minimumFractionDigits: 2 })}
            </span>
          </div>
          <div style={{ display: 'flex', justifyContent: 'space-between', padding: '10px 0', borderBottom: '1px solid #f0f0f0', fontSize: '13px' }}>
            <span style={{ color: '#666' }}>From</span>
            <span style={{ fontWeight: 600 }}>{transactionData.sender?.name} ({transactionData.sender?.email})</span>
          </div>
          <div style={{ display: 'flex', justifyContent: 'space-between', padding: '10px 0', borderBottom: '1px solid #f0f0f0', fontSize: '13px' }}>
            <span style={{ color: '#666' }}>To</span>
            <span style={{ fontWeight: 600 }}>{transactionData.receiver?.name} ({transactionData.receiver?.email})</span>
          </div>
          <div style={{ display: 'flex', justifyContent: 'space-between', padding: '10px 0', borderBottom: '1px solid #f0f0f0', fontSize: '13px' }}>
            <span style={{ color: '#666' }}>Status</span>
            <span style={{ fontWeight: 600, color: '#059669' }}>{transactionData.status}</span>
          </div>
          <div style={{ display: 'flex', justifyContent: 'space-between', padding: '10px 0', borderBottom: '1px solid #f0f0f0', fontSize: '13px' }}>
            <span style={{ color: '#666' }}>Date</span>
            <span style={{ fontWeight: 600 }}>{new Date(transactionData.createdAt).toLocaleString('en-US', { dateStyle: 'long', timeStyle: 'medium' })}</span>
          </div>
          <div style={{ fontSize: '10px', wordBreak: 'break-all', color: '#999', marginTop: '16px', padding: '8px', background: '#f9f9f9', borderRadius: '6px' }}>
            <strong>Integrity Hash (SHA-256):</strong><br/>{transactionData.receiptHash}
          </div>
          <div style={{ textAlign: 'center', marginTop: '24px', fontSize: '11px', color: '#999' }}>
            SecureBank™ – This is a computer-generated receipt.
          </div>
        </div>
      )}
